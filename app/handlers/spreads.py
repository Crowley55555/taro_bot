...
from app.services.session_service import ensure_defaults, reset_for_new_spread
...
from app.utils.text import escape_html, short_spread_summary
...
async def callback_spread_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await q.message.edit_text(
        ...
        f"Карт в раскладе: {sp.cards_count}. Сначала сформулируйте вопрос одним сообщением — "
        "после этого я вытяну карты и сделаю толкование.\n\n"
        f"(До {CONFIG.question_max_len} символов; короткая формулировка допустима, "
        "если вопрос понятен по смыслу.)",
        parse_mode="HTML",
    )
