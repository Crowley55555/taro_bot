...
def free_session_resume_keyboard() -> InlineKeyboardMarkup:
    """Повторный вход в свободный расклад при уже активной сессии."""

    return InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("Продолжить текущий", callback_data="fr:resume")],
            [InlineKeyboardButton("Новый расклад", callback_data="fr:newctx")],
            [InlineKeyboardButton("Главное меню", callback_data="m:main")],
        ]
    )


def free_session_actions_keyboard() -> InlineKeyboardMarkup:
    """Меню свободной сессии: карты, новый контекст, выход."""
...
def history_pagination_keyboard(*, page: int, has_next: bool) -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = []
    nav: list[InlineKeyboardButton] = []
    if page > 0:
        nav.append(InlineKeyboardButton("←", callback_data=f"hist:l:{page - 1}"))
    if has_next:
        nav.append(InlineKeyboardButton("→", callback_data=f"hist:l:{page + 1}"))
    if nav:
        rows.append(nav)
...
def reading_style_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("Обычный ИИ", callback_data="rs:normal_ai")],
            [InlineKeyboardButton("Предсказатель", callback_data="rs:predictor")],
            back_cancel_row("m:set"),
        ]
    )