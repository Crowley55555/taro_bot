...
def _is_transient_telegram_transport_error(err: BaseException) -> bool:
    """Сбои long polling / прокси к api.telegram.org — не баг приложения."""
...
async def _on_error(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if _is_transient_telegram_transport_error(err):
        _log.warning(
            "Временная ошибка сети Telegram (polling продолжит попытки): %s",
            err,
        )
        return
...
def build_application() -> Application:
    ...
    application.add_handler(CallbackQueryHandler(history.callback_history, pattern=r"^hist:"))
