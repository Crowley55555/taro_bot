...
def start_free_session(user_data: dict[str, Any], topic: str) -> None:
    ...
    user_data[KEY_FREE_SLIDING_MEMORY] = []
    user_data.pop(KEY_FREE_FSM, None)


def go_idle(user_data: dict[str, Any]) -> None:
    """Выход в главное меню: idle + сброс классического расклада. Свободная сессия сохраняется."""

    user_data.pop(KEY_FREE_PENDING_STEP_SIZE, None)
    if user_data.get(KEY_FREE_SESSION_ACTIVE) or user_data.get(KEY_SESSION_MODE) == SESSION_MODE_FREE:
        st = user_data.get(KEY_STATE)
        if st == STATE_FREE_SESSION_WAITING_QUESTION and not user_data.get(KEY_FREE_CURRENT_CARDS):
...
def get_reading_style(user_data: dict[str, Any]) -> ReadingStyle:
    ...
    if v in ("soft", "psychological", "practical", "normal_ai", "predictor"):
        return v  # type: ignore[return-value]
    return "soft"
...
def get_free_sliding_memory(user_data: dict[str, Any]) -> list[dict[str, str]]:
    h = user_data.get(KEY_FREE_SLIDING_MEMORY)
    if isinstance(h, list):
        return h  # type: ignore[return-value]
    return []


def append_free_sliding_pair(user_data: dict[str, Any], user_text: str, assistant_text: str) -> None:
    """Краткосрочная память свободной сессии (скользящее окно)."""

    mem = list(get_free_sliding_memory(user_data))
    ...
    if len(mem) > lim:
        mem = mem[-lim:]
    user_data[KEY_FREE_SLIDING_MEMORY] = mem
