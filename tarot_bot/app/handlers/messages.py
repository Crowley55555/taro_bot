...
from app.states import (
    ...
    STATE_CLASSIC_MINI_WAITING_QUESTION,
    STATE_FOLLOWUP_MODE,
...
async def handle_user_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == STATE_CLASSIC_MINI_WAITING_QUESTION:
        ok, err = validate_question_length(text)
        if not ok:
            await update.message.reply_text(err or "Некорректный текст.")
            return
        await _handle_classic_mini_question(update, context, text)
        return
...
async def _handle_classic_mini_question(update: Update, context: ContextTypes.DEFAULT_TYPE, question: str) -> None:
    """Вопрос к мини-раскладу (1/3/5 карт) после нажатия pr:* в классическом режиме."""

    cards_raw = context.user_data.get(KEY_CURRENT_CARDS)
    if not isinstance(cards_raw, list) or not cards_raw:
        await update.message.reply_text("Нет карт для шага. Нажмите 1/3/5 в меню после чтения.")
        context.user_data[KEY_STATE] = STATE_FOLLOWUP_MODE
        return
    ...
    title = str(context.user_data.get(KEY_CURRENT_SPREAD_TITLE) or "Мини-расклад")
    desc = str(context.user_data.get(KEY_CURRENT_SPREAD_DESCRIPTION) or "")
    ...
    try:
        ...
        _log.exception("Ошибка мини-расклада")
...
async def callback_post_reading_pick(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.user_data[KEY_CURRENT_CARDS] = serialize_cards(drawn)
    ...
    context.user_data[KEY_CURRENT_SPREAD_DESCRIPTION] = (
        "Случайные карты для следующего шага. Напишите вопрос к этому шагу одним сообщением."
    )
    context.user_data[KEY_STATE] = STATE_CLASSIC_MINI_WAITING_QUESTION

    body = format_cards_list_html(drawn)
    await q.message.reply_html(
        f"<b>Новый шаг: {size} карт</b>\n{body}\n\n"
...
async def callback_save_reading(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    storage.save_reading(
        ...
        spread_key=str(context.user_data.get(KEY_CURRENT_SPREAD_KEY) or ""),
        spread_title=str(context.user_data.get(KEY_CURRENT_SPREAD_TITLE) or ""),
        cards=cards,
        user_question=str(context.user_data.get(KEY_LAST_QUESTION) or ""),
        interpretation=interp,
        provider=str(context.user_data.get(KEY_PROVIDER) or ""),
        model=str(context.user_data.get(KEY_MODEL) or ""),
        reading_mode=str(context.user_data.get(KEY_READING_MODE) or "medium"),
        reading_style=str(context.user_data.get(KEY_READING_STYLE) or "soft"),
        reasoning_details=context.user_data.get(KEY_CURRENT_REASONING_DETAILS),
    )
    await q.message.reply_text("Расклад сохранён в истории.", reply_markup=main_menu())
